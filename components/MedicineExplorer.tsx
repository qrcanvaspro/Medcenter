...
const MedicineExplorer: React.FC = () => {
  ...
  const handleSearch = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'Medicine details not found. Please check the spelling.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="space-y-8 animate-in slide-in-from-bottom-8 duration-500">
          ...
          <div className="bg-white rounded-[3rem] border border-slate-100 shadow-2xl overflow-hidden">
            ...
            <div className="px-8 lg:px-12 pb-12 grid grid-cols-1 lg:grid-cols-3 gap-12">
              ...
              {/* Side Effects */}
              <div className="space-y-4">
                <h4 className="flex items-center gap-2 font-black text-slate-800 uppercase tracking-widest text-xs text-amber-600">
                  <ListRestart size={16} /> Common Side Effects
                </h4>
                <div className="flex flex-wrap gap-2">
                  {details.sideEffects.map((item: string, i: number) => (
                    <span key={i} className="bg-amber-50 text-amber-800 px-3 py-1.5 rounded-lg font-bold text-[11px] border border-amber-100">
                      {item}
                    </span>
                  ))}
                </div>
            ...
            <div className="bg-slate-900 p-8 text-center border-t border-slate-800">
              <div className="max-w-3xl mx-auto space-y-3">
                ...
                <p className="text-slate-400 text-[11px] leading-relaxed font-medium">
                  This information is generated by AI and intended for educational purposes only. It is NOT a substitute for professional medical advice, diagnosis, or treatment. <br />
                  <span className="text-white font-bold uppercase">Important: Pharmacy orders are managed directly by Mr. Manish Yadav to ensure authenticity and speed.</span>
                </p>
              </div>
            </div>
          </div>
